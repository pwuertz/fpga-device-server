        ...
        if (actual_length > 0)
        {
            ...
            if (tc->offset + actual_length <= tc->size)
            {
                ...
                if (tc->offset == tc->size)
                {
                    ...
                    tc->completed = 1;
                    if (tc->callback) tc->callback(tc, tc->user_data);
                    return;
                }
            }
            else
...
{
	return ftdi_write_data_submit_cb(ftdi, buf, size, 0, 0);
}
...
/**
    ...
    \param buf Buffer with the data
    \param size Size of the buffer
    \param cb Callback invoked on transfer completion
...
struct ftdi_transfer_control *ftdi_write_data_submit_cb(struct ftdi_context *ftdi, unsigned char *buf, int size, ftdi_transfer_cb_fn cb, void *user_data)
{
...
{
	return ftdi_read_data_submit_cb(ftdi, buf, size, 0, 0);
}
...
struct ftdi_transfer_control *ftdi_read_data_submit_cb(struct ftdi_context *ftdi, unsigned char *buf, int size, ftdi_transfer_cb_fn cb, void *user_data)
{
    struct ftdi_transfer_control *tc;
    struct libusb_transfer *transfer;
    int ret;
