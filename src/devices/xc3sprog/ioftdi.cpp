...
IOFtdi::IOFtdi(libusb_device* dev, ftdi_interface interface, int freq)
  : IOBase(), bptr(0), calls_rd(0), calls_wr(0), retries(0), device_has_fast_clock(true), tck_freq(30e6), buflen(0), subtype(0)
{
    use_ftd2xx = false;
    ...
    if (Init(dev, interface, freq) != 0)
    	throw std::runtime_error("Error initializing IOFtdi");
}
...
int IOFtdi::Init(libusb_device* dev, ftdi_interface interface, int freq)
{
  ...
  if (!use_ftd2xx)
  {
      ...
      res = ftdi_set_interface(ftdi_handle, interface);
      if(res <0)
      {
      ...
      // Open device
      res = ftdi_usb_open_dev(ftdi_handle, dev);
      if (res == 0)
      {
